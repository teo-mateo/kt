...
namespace kt.api.Controllers
{
    public class TestsController : ApiController
    {
        ...
        [Route("api/NewTest/{deckId}")]
        [HttpGet]
        public IHttpActionResult NewTest(int deckId)
        {
            var test = Test.CreateNew(db, deckId);
            db.Tests.Add(test);
            db.SaveChanges();
            ...
            {
                TestId = test.Id,
                CardsCount = test.Cards.Count,
                Cards = test.Cards.Select(p=>p.Card)
            });
        }
